...
# fois le même style.
EXTRA_KPI_PILLS = [
    ("taux_neutre", "Taux de Neutres", _fmt_pct, "E0E0E0", "1a1a1a"),
    ("score_moyen", "Score Moyen", _fmt_score, "FFE082", "1a1a1a"),
    ("duree_moyenne_min", "Durée Moyenne", _fmt_min, "D1C4E9", "1a1a1a"),
    ("taux_commentaires", "Taux Commentaires", _fmt_pct, "B2DFDB", "1a1a1a"),
    ...
    try:
        ...
        if ttype == "bar" and len(traces) > 1:
            ...
            for i, tr in enumerate(traces):
                ...
                ax.bar_label(bars, fontsize=FS_LABEL - 2, padding=2)
            ...
            if orientation == "h":
                ...
                bars = ax.barh(categories_clean, xs, color=colors)
                ax.invert_yaxis()
                ax.bar_label(bars, fontsize=FS_LABEL, padding=4)
                ax.tick_params(axis="y", labelsize=FS_TICK)
...
def prerender_agency_page_images(pages: list[dict], progress_callback=None) -> None:
    ...
    for i, (page, chart) in enumerate(todo):
        figure_to_png_bytes(chart["fig"], width=1100, height=650, scale=1.5)
...
def build_pptx_report(report_text: str, analysis_log: list[dict] | None = None,
    ...
    try:
        ...
        p3.text = f"Généré le {datetime.now():%d/%m/%Y à %H:%M}"
        p3.font.size = Pt(16); p3.font.color.rgb = BK
...
def build_agency_report_pdf(pages: list[dict], meta: dict | None = None, pill_keys: list[str] | None = None) -> tuple[bool, bytes | str]:
    ...
    try:
        ...
        story.append(Paragraph(f"Généré le {datetime.now():%d/%m/%Y à %H:%M}",
                                ParagraphStyle("CoverGen", parent=styles["Normal"], alignment=1, fontSize=10,
        ...
        for page in pages:
            ...
            for chart in page.get("all_charts") or []:
                img_bytes = figure_to_png_bytes(chart["fig"], width=1500, height=850, scale=2.0, show_title=False)
                if img_bytes:
                    if chart.get("title"):
                        figure_counter[0] += 1
...
def build_agency_report_word(pages: list[dict], meta: dict | None = None, pill_keys: list[str] | None = None) -> tuple[bool, bytes | str]:
    ...
    try:
        ...
        p = document.add_paragraph(f"Généré le {datetime.now():%d/%m/%Y à %H:%M}")
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def build_agency_report_pptx(pages: list[dict], meta: dict | None = None,
    ...
    try:
        ...
        gen = slide.shapes.add_textbox(Inches(0), Inches(5.9), prs.slide_width, Inches(0.4))
        gp = gen.text_frame.paragraphs[0]; gp.text = f"Généré le {datetime.now():%d/%m/%Y à %H:%M}"
        gp.font.size = Pt(10); gp.font.color.rgb = PptxRGB.from_string("888888"); gp.alignment = PP_ALIGN.CENTER
        ...
        for page in pages:
            ...
            for chart in page.get("all_charts") or []:
                ...
                img_bytes = figure_to_png_bytes(chart["fig"], width=1100, height=600, scale=1.5)
...
def build_generic_report_pdf(charts: list[dict], dataset_kpis: dict, meta: dict | None = None,
    ...
    try:
        ...
        story.append(Spacer(1, 0.6 * cm))
        story.append(Paragraph(f"Généré le {datetime.now():%d/%m/%Y à %H:%M}",
                                ParagraphStyle("CoverDateG", parent=styles["Normal"], fontSize=10,
...
def build_generic_report_word(charts: list[dict], dataset_kpis: dict, meta: dict | None = None,
    ...
    try:
        ...
        document.add_paragraph()
        dp = document.add_paragraph(); dp.alignment = WD_ALIGN_PARAGRAPH.CENTER
        dr = dp.add_run(f"Généré le {datetime.now():%d/%m/%Y à %H:%M}"); dr.font.size = Pt(10); dr.font.color.rgb = RGBColor.from_string("666666")
        document.add_page_break()
